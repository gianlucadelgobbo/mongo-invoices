				...
				DB.invoices.findOne({_id:new ObjectID(req.query.id)},function(e, result) {
					...
					fs.writeFile('./app/public/accounts/'+global.settings.dbName+"/style_print.jade", "", { flag: 'wx' }, function (err) {
						res.render('../../public/accounts/'+global.settings.dbName+"/style_print", {layout: false}, function (error_style, style) {
							res.render('print_invoice', {locals: {	title: __("Invoice"), country:global._config.company.country, result : result, udata : req.session.user, file:folder+filename, style:style } }, function (error1, html1) {
								// PDF START
