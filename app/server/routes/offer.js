        ...
        DB.offers.findOne({_id:new ObjectID(req.query.id)},function(e, result) {
          ...
          DB.customers.findOne({_id:new ObjectID(result.to_client._id)},function(e, to_client) {
            ...
            res.render('accounts/'+global.settings.dbName+"/style_print", {layout: false}, function (error_style, style) {
              res.render('offer_preview', {  title: __("Offer"), country:global._config.company.country, result : result, udata : req.session.user, file:folder+filename, style:style, js:"/js/sendemail.js", to_client:to_client}, function (error1, html1) {
                // PDF START
