...
function sendByEmail() {
	$('.modal-sendbyemail .modal-title').html("Send by email 2");
	showModal("sendbyemail", "Loading...");
	/*$.ajax({
		'async': false,
		url: "/api/offers",
		dataType: "json",
	...
	$.ajax({
		...
		success: function( data ) {
			if (data.result.length) {
				...
				showModal('error', "La data deve essere uguale o successiva al "+d.getDate()+"/"+(d.getMonth()+1)+"/"+d.getFullYear(), function () {setTimeout("\$(\"#invoice_date\").focus()",50)});
			}
		}
	});
}
